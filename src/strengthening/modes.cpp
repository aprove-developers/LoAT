...
namespace strengthening {
    ...
    const std::vector<Mode> Self::modes() {
        return {invariance, pseudoInvariance, monotonicity};
    }
    ...
    const MaxSmtConstraints Self::monotonicity(const SmtConstraints &constraints, Z3Context &z3Ctx) {
        ...
        if (!constraints.continuation.satisfiable.empty()) {
            z3::expr_vector satisfiableForSomeSuccessor(z3Ctx);
            for (const z3::expr &e: constraints.continuation.satisfiable) {
                satisfiableForSomeSuccessor.push_back(e);
            }
            res.soft.push_back(z3::mk_or(satisfiableForSomeSuccessor));
        }
        res.hard.insert(res.hard.end(), constraints.templatesInvariant.begin(), constraints.templatesInvariant.end());
        return res;
    }
