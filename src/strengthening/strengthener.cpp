...
namespace strengthening {
    ...
    const std::vector<Rule> Self::apply(const Rule &r, ITSProblem &its) {
        ...
        while (!todo.empty()) {
            ...
            const Strengthener &strengthener = Strengthener(current, its);
    ...
    Self::Strengthener(const Rule &r, ITSProblem &its) :
    ...
    const std::vector<Rule> Self::apply(const Types::Mode &mode) const {
        std::vector<Rule> res;
        if (r.isSimpleLoop()) {
            ...
            if (!mon.failed.empty()) {
                ...
                const option<Types::Invariants> &newInv = tryToForceInvariance(inv.solved, mon.failed, preconditions,
                                                                               mode);
                if (newInv) {
                    ...
                    for (const Expression &e: newInv.get().pseudoInvariants) {
                        ...
                        res.emplace_back(
                                Rule(RuleLhs(r.getLhsLoc(), pseudoInvariantInvalid, r.getCost()), r.getRhss()));
                    }
                }
            }
        }
