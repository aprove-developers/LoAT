...
struct RuntimeResultNL {
    ...
    RuntimeResultNL() : cpx(Complexity::Unknown), bound(0), reducedCpx(false) {}
};
...
public:
    ...
    static RuntimeResultNL analyze(ITSProblem &its, AnalysisSettings cfg);


private:
    NonlinearITSAnalysis(ITSProblem &its, AnalysisSettings cfg);
    ...
    // Nonlinear algorithm
    bool partialDeletion(TransIdx ruleIdx, LocationIdx loc);
    bool removeSinkRhss();
     ...
     */
    RuntimeResultNL getMaxPartialResult();
