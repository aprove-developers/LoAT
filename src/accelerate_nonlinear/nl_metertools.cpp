...
void MeteringToolboxNL::applySubsToUpdates(const GiNaC::exmap &subs, MultiUpdate &updates) {
...
bool MeteringToolboxNL::isUpdatedByAny(VariableIdx var, const MultiUpdate &updates) {
...
GuardList MeteringToolboxNL::replaceEqualities(const GuardList &guard) {
    GuardList newGuard;
...
GuardList MeteringToolboxNL::reduceGuard(const VarMan &varMan, const GuardList &guard, const MultiUpdate &updates,
    ...
    for (Expression ex : guard) {
        ...
        if (forceAdd) {
            ...
            for (const UpdateMap &update : updates) {
                ...
                // unsat means that the updated `ex` must always hold (so is implied after the update)
                if (z3res != z3::unsat) {
                    implied = false;
                    break;
                }
...
set<VariableIdx> MeteringToolboxNL::findRelevantVariables(const VarMan &varMan, const GuardList &guard, const MultiUpdate &updates) {
    set<VariableIdx> res;
...
void MeteringToolboxNL::restrictUpdatesToVariables(MultiUpdate &updates, const set<VariableIdx> &vars) {
    for (UpdateMap &update : updates) {
...
void MeteringToolboxNL::restrictGuardToVariables(const VarMan &varMan, GuardList &guard, const set<VariableIdx> &vars) {
...
bool MeteringToolboxNL::strengthenGuard(const VarMan &varMan, GuardList &guard, const MultiUpdate &updates) {
...
stack<GiNaC::exmap> MeteringToolboxNL::findInstantiationsForTempVars(const VarMan &varMan, const GuardList &guard) {
